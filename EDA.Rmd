...
year_mean_df <- data.frame(year=2009:2022,temp_mean=year_mean,longterm_mean=long_mean)
year_mean_df
```
...
```{r}
year_mean <- data.frame(year=2009:2022,temp_mean=year_mean,longterm_mean=rep(long_mean[1],14))
year_mean
```
...
```{r}
year_mean %>%  ggplot() + geom_line(aes(x=year,y=temp_mean),col= "#C11432") + geom_line(aes(x=year,y=longterm_mean),col="#009ADA") + theme_minimal() + xlab("") + 
